...
model_interface_train<-function(input_tra){
  ...
  if(length(return_post_model)==3){
    #存入数据库
    write.csv(return_post_model$result_bd,paste0(price_model_loc,"/output/result/yck_project_model_result_tag",".csv"),
              row.names = F,fileEncoding = "UTF-8",quote = F)
    write.csv(return_post_model$result_compare,paste0(price_model_loc,"/output/result/yck_project_model_result_match",".csv"),
              row.names = F,fileEncoding = "UTF-8",quote = F)
    write.csv(return_post_model$result_future,paste0(price_model_loc,"/output/result/yck_project_model_result_future",".csv"),
              row.names = F,fileEncoding = "UTF-8",quote = F)
...
model_interface_train_yck<-function(input_tra){
  ...
  yck_project_model_query<-data.frame(user_query_id=max_user_query_id+1,before_query,query_statue=1)
  yck_project_model_query$add_time<-as.character(format(Sys.time(),"%Y/%m/%d %H:%M:%S"))
  yck_project_model_query<-yck_project_model_query %>% dplyr::select(user_query_id,user_id,select_tname,yck_car_id,query_number,select_model_id,select_regDate,select_mile,select_partition_month,
                                                                     select_classification_operational,select_classification_car,add_time,query_statue)
  write.csv(yck_project_model_query,paste0(price_model_loc,"/output/result/yck_project_model_query",".csv"),
            row.names = F,fileEncoding = "UTF-8",quote = F)
  dbSendQuery(loc_channel,paste0("LOAD DATA LOCAL INFILE ","'",paste0(price_model_loc,"/output/result/yck_project_model_query",".csv"),"'",
  ...
  if(length(return_post_model)==3){
    ...
    dbSendQuery(loc_channel,'SET NAMES gbk')
    dbSendQuery(loc_channel,paste0("LOAD DATA LOCAL INFILE ","'",paste0(price_model_loc,"/output/result/yck_project_model_result_match",".csv"),"'",
                                   " INTO TABLE yck_project_model_n_result_match CHARACTER SET utf8 FIELDS TERMINATED BY ',' lines terminated by '\r\n' IGNORE 1 LINES;"))
    dbSendQuery(loc_channel,paste0("LOAD DATA LOCAL INFILE ","'",paste0(price_model_loc,"/output/result/yck_project_model_result_tag",".csv"),"'",
                                   " INTO TABLE yck_project_model_n_result_tag CHARACTER SET utf8 FIELDS TERMINATED BY ',' lines terminated by '\r\n' IGNORE 1 LINES;"))
    dbSendQuery(loc_channel,paste0("LOAD DATA LOCAL INFILE ","'",paste0(price_model_loc,"/output/result/yck_project_model_result_future",".csv"),"'",
                                   " INTO TABLE yck_project_model_n_result_future CHARACTER SET utf8 FIELDS TERMINATED BY ',' lines terminated by '\r\n' IGNORE 1 LINES;"))
...
#车源估值调用（简版）
model_interface_train_yckit<-function(input_tra){
  #数据输入：#
  before_query <-input_tra
  max_user_query_id<-read.table(paste0(price_model_loc,"/file/max_user_query_id.txt")) %>% as.character() %>% as.integer()
  write.table(max_user_query_id+1,paste0(price_model_loc,"/file/max_user_query_id.txt"),row.names = F,col.names = F,append = F)
