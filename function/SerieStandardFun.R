...
carMatchFun<-function(input_modelname,car_id){
  ...
  input_filter<-rbind(input_filter[input_filter$series_name!=car_select$series_name,],linshi)
  
  ##去掉部分字段
  input_filter_tr<-input_filter%>%dplyr::select(-model_id,-brand_name,-series_name,-model_name,-car_level,-auto)
  ##对标：第二部分：算法计算##
  #距离变换
  ...
  dis_series <- tsne_obj$Y %>%data.frame() %>%setNames(c("X", "Y")) %>%
    dplyr::mutate(model_id=input_filter$model_id,brand_name = input_filter$brand_name,series_name = input_filter$series_name)
  ...
  #   dplyr::mutate(cluster = factor(result$cluster),brand_name = input_filter$brand_name,series_name = input_filter$series_name,name = input_filter$model_name)
