...
main_fun_series_standard<-function(car_id){
  ...
  output_mathc_fin<-data.frame(bd_model_id=car_id,model_id=car_id,add_time=as.character(Sys.Date()))
  if(nrow(output_error)!=0){
    ...
    output_mathc_fin<-tryCatch(carMatchFun(input_modelname,car_id),
                               error=function(e){output_mathc_fin},
                               finally={print("对标OK!")})
  }
  fun_mysqlload_query(local_defin,paste0("DELETE FROM config_match_modelstandard WHERE bd_model_id=",car_id))
  #fun_mysqlload_add(price_model_loc,local_defin,output_mathc_fin,'config_match_modelstandard')
  fun_mysqlload_query_add(local_defin,'config_match_modelstandard',output_mathc_fin)
}
...
carMatchFun<-function(input_modelname,car_id){
  ...
  ##对标：第三部分：结果分析##
  output_result<-sc %>% dplyr::mutate(bd_model_id=car_id,add_time=as.character(Sys.Date())) %>% 
    dplyr::filter(cc==cc[which(model_id==car_id)]) %>% dplyr::select(bd_model_id,model_id,add_time)
  output_result2<-sc2 %>% dplyr::mutate(bd_model_id=car_id,add_time=as.character(Sys.Date())) %>% 
    dplyr::filter(cc==cc[which(model_id==car_id)]) %>% dplyr::select(bd_model_id,model_id,add_time)
  output<-rbind(output_result,output_result2) %>% unique()
  return(output)
}