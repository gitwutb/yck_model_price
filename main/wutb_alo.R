...
model_algorithm1<-function(parameter_gamma){
  model.svm <- e1071::svm(quotes~., input_train[1:(nrow(input_train)-300),],gamma=parameter_gamma)
  train_output<-data.frame(input_train[1:(nrow(input_train)-300),],pre_preds=model.svm$fitted)
  pre_preds <- predict(model.svm, input_train[(nrow(input_train)-299):nrow(input_train),-4]) 
  test_output<-data.frame(input_train[(nrow(input_train)-299):nrow(input_train),],pre_preds)
...
error_return<-apply(array(seq(0.008,0.15,by=0.003), c(1,length(seq(0.008,0.15,by=0.003)))), 2, model_algorithm1)
error_return<-list.rbind(error_return)
ggplot(data=error_return, mapping=aes(x=parameter_gamma, y=x1))+geom_line(aes(color='y'))+
  geom_line(aes(y=x2,color='r'))


########算法二：nnet中的神经网络####
model.svm <- nnet::nnet(quotes~., input_train[1:(nrow(input_train)-300),], size = 5, decay = 0.01, maxit = 1000, linout = T, trace = F) 
train_output1<-data.frame(input_train[1:(nrow(input_train)-300),],pre_preds=model.svm$fitted)
pre_preds <- predict(model.svm, input_train[(nrow(input_train)-299):nrow(input_train),-4]) 
test_output1<-data.frame(input_train[(nrow(input_train)-299):nrow(input_train),],pre_preds)
#评价
mean(abs(model.svm$residuals))
mean(abs(test_output1$pre_preds-test_output1$quotes))
train_output1%>%group_by(car_platform)%>%summarise(mean(abs(pre_preds-quotes)))%>%as.data.frame()
ggplot(train_output1,aes(1:nrow(train_output1),pre_preds-quotes,color=car_platform))+geom_point()
